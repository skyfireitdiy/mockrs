...
- 改造成本：
  - Trait 模拟：常需为可测试性重构生产代码（抽象接口、依赖注入）。
  - mockrs：直接作用于已存在的自由函数/具体实现，无需为测试重构设计。
- 可模拟对象：
  - Trait 模拟：以接口为中心，难以覆盖自由函数与部分 FFI。
  - mockrs：自由函数、具体方法、FFI 均可（前提是有可寻址入口）。
- 线程隔离：
  - Trait 模拟：通常为进程级替换，难以自然隔离线程。
  - mockrs：TLS 驱动的线程级生效，天然支持并发测试的相互独立。
- API 复杂度：
  - Trait 模拟：需要定义/实现 Trait，配置期望与行为。
  - mockrs：一个 mock! 宏即可完成替换，RAII 自动恢复。
- 典型场景：
  - Trait 模拟：单元测试、面向接口的业务逻辑隔离。
  - mockrs：集成测试、边界条件/自由函数/FFI/系统调用包装等难以接口化的路径。
...
#### 6.1.1 当函数首条指令为跳转（jmp/call/jcc）时的行为（x86_64）
...
#### 6.2.1 为什么 aarch64 没有单步/写回抑制机制？是否不需要？
